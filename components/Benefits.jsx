  ...
  return (
    <Section id="features">
      <div className="container relative z-2">
        <Heading className="md:max-w-md large:max-w-2xl" title="Chat smarter, not harder"></Heading>
                ...
                <div className="block relative p-0.5 bg-no-repeat bg-[length:100%_100%] md:max-w-[24rem]" style={{backgroundImage: `url(${item.backgroundUrl})`}} key={item.id}>
                    <div className="relative z-2 flex flex-col min-h-[22rem] p-[2.4rem] pointer-events-none">
                        ...
                        <div className='flex items-center mt-auto'>
                          <Image src={item.iconUrl} alt={item.title} width={48} height={48}/>
                    ...
                    <div className='absolute inset-0.5 bg-n-8' style={{clipPath: "url(#benefits)"}}>
                      <div className='absolute inset-0 opacity-0 transition-opacity hover:opacity-10'>
                        {item.imageUrl && (
                          <Image src={item.imageUrl} alt={item.title} width={380} height={362} className='w-full h-full object-cover'/>
                        )}
                      </div>
                    </div>
                    <ClipPath/>
